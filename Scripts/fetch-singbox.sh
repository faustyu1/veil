...
TAG="$(resolve_tag "${REPO}" sing-box "${ARCH}")" || {
  echo "Could not resolve a sing-box release tag." >&2
  exit 1
}
echo "sing-box release: ${TAG}"
...
verify_asset sing-box "${ARCH}" "${TMP}/singbox.tar.gz" || exit 1
record_asset sing-box "${ARCH}" "${TAG}" "${TMP}/singbox.tar.gz"
